...
func SignUpHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		accessToken, err := security.GenerateAccessToken(&users)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"comment": "gentoken faild",
			})
			return
		}
		refreshToken, err := security.GenerateRefreshToken(&users)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"comment": "gentoken faild",
			})
...
func SignInHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		refreshToken, err := security.GenerateRefreshToken(data)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"comment": "gentoken faild",
			})
		...
		c.JSON(http.StatusOK, common.ReponseDataToken("Đăng nhập thành công", nil, refreshToken))
	}
}
...
func ProfileUser(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		biz := users_bussiness.NewProfileController(service.NewSql(db))
...
func UpdateUserHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		biz := users_bussiness.NewUpdateUserController(service.NewSql(db))
...
func DeletedUserHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		biz := users_bussiness.NewDeletedUserService(service.NewSql(db))
		fmt.Printf("biz: %v\n", biz)
